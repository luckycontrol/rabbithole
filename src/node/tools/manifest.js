...
export const toolDefinitions = [
  ...
  },
  {
    name: "answer_branch",
    description: [
      "Answer one pending branch_request, revision_request, or convert_request from an open Rabbithole. For revision_request without a 'selection' field, return a complete replacement for the existing card according to its instruction; the human previews it before applying, and no new branch is created. For revision_request WITH a 'selection' field, the human highlighted one passage: return only the replacement Markdown for selection.region_markdown, with no title line, no surrounding code fence, no commentary, and none of the rest of the card — the host splices your reply into that exact source span, so anything extra lands in the document verbatim. For convert_request, read every pages[].image_path in order, follow rules exactly, stream transcription chunks, and emit figure: refs rather than cropping. For branch_request, write a focused answer using the supplied selection context; when region.image_path is present, it may be the new selection clip or the immediate parent's clip, so read it and trust it over extracted text.",
      "",
