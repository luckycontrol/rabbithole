...
async function runHoleContract(store) {
  ...
  const raw = JSON.parse(await fs.readFile(path.join(process.env.RABBITHOLE_DIR, "contract-hole.json"), "utf8"));
...
async function runMigrationContract(store) {
  const fixture = {
    hole_id: "legacy-hole",
    title: "Legacy Hole",
    root_id: "root",
  ...
  await fs.writeFile(path.join(process.env.RABBITHOLE_DIR, "legacy-hole.json"), JSON.stringify(fixture, null, 2), "utf8");
  ...
  const saved = JSON.parse(await fs.readFile(path.join(process.env.RABBITHOLE_DIR, "legacy-hole.json"), "utf8"));
...
async function runAssetGcFixture(store) {
...
await runHoleContract(store);
await runMigrationContract(store);
await runAssetContract(store);
await runStagingContract(store);
await runSafetyContract(store);
await runAssetGcFixture(store);
